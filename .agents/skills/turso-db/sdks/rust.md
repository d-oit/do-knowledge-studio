...
async fn main() {
    ...
    conn.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)", ()).a
wait.unwrap();
    conn.execute("INSERT INTO users (name) VALUES (?1)", ("Alice",)).await.unwra
p();

    let mut rows = conn.query("SELECT * FROM users", ()).await.unwrap();
    while let Some(row) = rows.next().await.unwrap() {
        println!("id={}, name={}", row.get_value(0).unwrap(), row.get_value(1).u
nwrap());
    }
}
```
...
```rust
let mut rows = conn.query("SELECT id, name FROM users WHERE id > ?1", (0,)).awai
t?;
...
- Use `()` for no parameters, tuples for positional params: `(?1, ?2)` → `(val1,
 val2)`
