...
The sync variant `@tursodatabase/sync-wasm` adds bidirectional sync with Turso C
loud:
...
SharedArrayBuffer is required for WASM threading. Your server must set these hea
ders:
...
If you use Vite, just add the headers to `vite.config.ts` — no extra server setu
p, proxies, or middleware needed for development:
...
Use the `/vite` subpath import for Vite projects — it handles WASM module and wo
rker loading correctly in Vite's dev server (works around known Vite issues with
 WASM + workers):
...
In production builds, the `/vite` import resolves to the default entry point. In
 development, it uses a special workaround that inlines the WASM binary to avoid
...
| Turbopack | `@tursodatabase/database-wasm/turbopack` | `@tursodatabase/sync-wa
sm/turbopack` |

## Notes

- Requires browser with SharedArrayBuffer support
- COOP/COEP headers are mandatory — without them, SharedArrayBuffer is unavailab
le
- OPFS is only available in secure contexts (HTTPS or localhost)
- The WASM package targets `wasm32-wasip1-threads`
- Install canary releases with `npm i @tursodatabase/database-wasm@next` or `npm
 i @tursodatabase/sync-wasm@next` for preview/experimental features
