...
const transfer = db.transaction(async (from, to, amount) => {
    await db.prepare('UPDATE accounts SET balance = balance - ? WHERE id = ?').r
un(amount, from);
    await db.prepare('UPDATE accounts SET balance = balance + ? WHERE id = ?').r
un(amount, to);
});
...
Execute and return info object with `changes` (modified row count) and `lastInse
rtRowid`.
...
- API is the async variant of `better-sqlite3` — all execution methods (`run`, `
get`, `all`, `iterate`, `exec`, `close`) are async and must be awaited
- Install canary releases with `npm i @tursodatabase/database@next` for preview/
