...
func main() {
    ...
    if _, err := db.Exec("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)
"); err != nil {
        log.Fatal(err)
    }
    if _, err := db.Exec("INSERT INTO users (name) VALUES (?)", "Alice"); err !=
 nil {
        log.Fatal(err)
    }
...
func main() {
    ...
    if _, err := db.Exec("INSERT INTO todos (title) VALUES (?)", "Buy groceries"
); err != nil {
        log.Fatal(err)
    }
    if _, err := db.Exec("INSERT INTO todos (title) VALUES (?)", "Write code");
err != nil {
        log.Fatal(err)
    }
    ...
    if _, err := db.Exec("UPDATE todos SET done = 1 WHERE id = ?", 1); err != ni
l {
        log.Fatal(err)
    }
}
...
- Blank import (`_ "turso.tech/database/tursogo"`) registers the `"turso"` drive
r
