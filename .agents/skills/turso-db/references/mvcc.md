...
Turso supports MVCC for concurrent read/write transactions with snapshot isolati
on.
...
MVCC enables `BEGIN CONCURRENT` transactions that allow multiple concurrent read
ers and writers:
...
4. At COMMIT time, conflict detection runs:
 ...
 → `SQLITE_BUSY` error
   - If an exclusive transaction (BEGIN IMMEDIATE) is active → `SQLITE_BUSY` err
or
...
- The row was modified by a transaction that committed after this transaction's
begin timestamp
...
- Concurrent transactions CAN read and write while an exclusive transaction is a
ctive
- Concurrent transactions CANNOT commit while an exclusive transaction holds the
 lock
- Use `BEGIN IMMEDIATE` only when you need exclusive access (e.g., schema change
s)

**Best practice:** For maximum concurrency, use `BEGIN CONCURRENT` for all write
 transactions.
