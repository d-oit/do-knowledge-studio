...
- `hexkey` — encryption key in hexadecimal (32 or 64 hex characters depending on
 cipher)
...
- Page 1 header (first 100 bytes) is NOT encrypted but IS authenticated (used as
 Additional Data)
...
- Cipher information is stored in the database file header (replacing SQLite mag
ic bytes)

## Remote Encryption (SDK)

SDKs support encrypting data synced to Turso Cloud via `remoteEncryption` config
uration. Remote encryption uses the same cipher algorithms but key format varies
 by SDK:
