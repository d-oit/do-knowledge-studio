 ...
 * `translate` helper bound to its own keys — no central index, so parallel
 * workstreams never race on a shared registry. A future `next-intl`/
 * `react-i18next` layer can aggregate the per-scope maps additively.
 *
 ...
 *   // <input placeholder={translate('editor.mention.placeholder')} />
 *
...
/**
 * Builds a typed `translate` helper for a const message map.
