...
export const EncryptExportDialog = memo(function EncryptExportDialog({
  ...
  return (
    <Overlay
      ...
      aria-label={t('export.encrypt.ariaLabel')}
    >
      <motion.div
      ...
      >
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-serif text-[15px] font-semibold text-ink">{t('export.encrypt.title')}</h3>
            <p className="text-label text-ink-faint">
              {t('export.encrypt.subtitle')}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div>
            <label htmlFor="encrypt-password" className="mb-1 block text-label font-semibold uppercase tracking-wide text-ink-faint">
              {t('export.encrypt.passwordLabel')}
            </label>
            <div className="relative">
              <input
                ...
                placeholder={t('export.encrypt.passwordPlaceholder')}
              ...
              <button
                ...
                aria-label={showPass ? t('export.encrypt.hidePassword') : t('export.encrypt.showPassword')}
              >
                {showPass ? t('export.encrypt.hide') : t('export.encrypt.show')}
              </button>
            </div>
          </div>
          <div>
            ...
            <input
              ...
              placeholder={t('export.encrypt.confirmPlaceholder')}
              aria-invalid={Boolean(password && confirm && password !== confirm)}
              aria-describedby={password && confirm && password !== confirm ? 'password-mismatch-error' : undefined}
          ...
          {password && confirm && password !== confirm && (
            <p id="password-mismatch-error" className="text-label text-red-500">{t('export.encrypt.mismatch')}</p>
          )}
        </div>
