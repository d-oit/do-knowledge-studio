...
import {
  narrowSemanticCorpus,
  resolveSemanticEntities,
  SemanticStatusBanner,
  useSemanticSearch,
} from './library-semantic-search'
import {
  FileText,
  LayoutGrid,
  List as ListIcon,
...
export const LibraryView = () => {
  ...
  // The retrieval corpus is narrowed by type BEFORE the query (see
  // `narrowSemanticCorpus`), so a type filter cannot be starved by the
  // retriever's global top-N truncation.
  const semanticCorpus = useMemo(
    () => narrowSemanticCorpus(allEntities, claims, typeFilter),
    [allEntities, claims, typeFilter],
  )
  const { semanticOutcome, semanticBusy } = useSemanticSearch(
    semanticMode,
    searchQuery,
    semanticCorpus.entities,
    semanticCorpus.claims,
  )
