    ...
    >
      <div className="mb-2 flex items-start justify-between">
        ...
        <button
          ...
          aria-label={t('triz.results.copyAria')}
        >
          {copied === id ? (
      ...
      {examples.length > 0 && (
        <div className="mt-auto border-t border-border pt-2">
          <p className="mb-1 text-caption font-semibold uppercase tracking-wide text-ink-faint">{t('triz.results.examples')}</p>
...
const ResultsSummaryDescription = ({
  ...
  <p className="mt-3 text-[13px] leading-relaxed text-ink-mute">
    {t('triz.results.summaryPrefix')}<strong className="text-ink-soft">{TRIZ_PARAMETERS.at(improving)?.toLowerCase() ?? ''}</strong>
    {t('triz.results.summaryMiddle')}<strong className="text-ink-soft">{TRIZ_PARAMETERS.at(worsening)?.toLowerCase() ?? ''}</strong>
    {t('triz.results.summarySuffix')}
    {hasSuggestions ? t('triz.results.summarySuggestions') : t('triz.results.summaryNone')}
  </p>
)
