...
export const CHAT_WELCOME_DESCRIPTION =
  'Ask questions about your library, find connections, or start a synthesis. Your content stays in this browser.'
/** Section heading for the capabilities list. */
export const CHAT_CAPABILITIES_LABEL = 'What you can ask'
/** Section heading for the prompt suggestion chips. */
export const CHAT_SUGGESTION_LABEL = 'Try a prompt'

/** Label prefix for the inline suggestion chips. */
export const SUGGESTIONS_LABEL = 'Try:'
/** Status indicator label for the local search engine. */
export const LOCAL_SEARCH_STATUS = 'Local search active'
/** Keyboard shortcut hint shown below the input. */
export const CHAT_SHORTCUT_HINT = 'Enter to send · Shift+Enter for newline'
...
export const TypingIndicator = ({ reducedMotion }: { reducedMotion?: boolean }) => (
  <div className="flex gap-3" aria-live="polite" aria-label="Assistant is typing">
    ...
    <div className="rounded-2xl rounded-tl-sm border border-border bg-card px-4 py-3">
      <div className="flex items-center gap-1.5">
        <span
          className={cn(
            'h-1.5 w-1.5 rounded-full bg-ink-faint [animation-delay:-0.3s]',
            reducedMotion ? '' : 'animate-bounce',
          )}
        />
        <span
          className={cn(
            'h-1.5 w-1.5 rounded-full bg-ink-faint [animation-delay:-0.15s]',
            reducedMotion ? '' : 'animate-bounce',
          )}
        />
        <span
          className={cn('h-1.5 w-1.5 rounded-full bg-ink-faint', reducedMotion ? '' : 'animate-bounce')}
        />
      </div>
    </div>
  </div>
...
export const WelcomePanel = ({ reducedMotion, onSend }: WelcomePanelProps) => (
  ...
  >
    ...
    <h2 className="mb-2 font-serif text-lg font-semibold text-ink">Ask your library</h2>
    <p className="mb-4 max-w-sm text-[14px] text-ink-mute">{CHAT_WELCOME_DESCRIPTION}</p>
          ...
          {citations.map((c, i) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-ink group-hover:text-saffron-deep">
                  {c.entityName}
                </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {input.length > COUNTER_THRESHOLD && (
              <span
                className={cn(
                  'font-mono',
                  input.length >= MAX_INPUT_LENGTH ? 'text-clay' : 'text-ink-faint',
                )}
              >
