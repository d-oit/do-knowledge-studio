...
```bash
npm run cli -- sync ./path/to/markdown/files
```
...
**Markdown:**
```bash
npm run cli -- export --format md --output ./export/markdown
```

**JSON:**
```bash
npm run cli -- export --format json --output ./export/json
```

**Static Site:**
```bash
npm run cli -- export --format site --output ./export/site
```

### Manage Entities and Claims
```bash
# Create an entity
npm run cli -- entity-create "Artificial Intelligence" --type concept --description "The simulation of human intelligence by machines."

# List entities
npm run cli -- entity-list

# Create a claim for an entity
npm run cli -- claim-create "Artificial Intelligence" "AI can solve complex problems" --confidence 0.95
```
