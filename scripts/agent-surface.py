...
class AgentSurfaceManager:
    ...
    def _sync_symlinks(self, target_dir: Path):
        ...
        for skill_path in self.canonical_skills_dir.iterdir():
            ...
            rel_target = os.path.relpath(skill_path, target_dir)
    ...
    def validate(self):
        ...
        # 1. Validate Canonical Skills
        errors += self._validate_canonical_skills()

        # 2. Validate Tool Directories and Symlinks
        for tool_name, config in self.manifest["tools"].items():
            tool_dir = self.root_dir / config["directory"]
            if not tool_dir.exists():
                log_error(f"Tool directory missing for {tool_name}: {config['directory']}")
                errors += 1
                continue
    ...
    def _validate_canonical_skills(self) -> int:
        errors = 0
        if not self.canonical_skills_dir.exists():
            log_error(f"Canonical skills directory missing: {self.canonical_skills_dir}")
            return 1
        ...
        for skill_path in self.canonical_skills_dir.iterdir():
            ...
            if line_count > self.max_skill_lines:
                log_error(f"Skill {skill_name}: SKILL.md exceeds {self.max_skill_lines} lines ({line_count})")
                errors += 1
    ...
    def _validate_tool_symlinks(self, target_dir: Path, tool_name: str) -> int:
        errors = 0
        for skill_path in self.canonical_skills_dir.iterdir():
            if not skill_path.is_dir() or skill_path.name.startswith("_"):
                continue
            ...
            if not link_name.exists():
                log_error(f"Missing skill in {tool_name}: {link_name}")
                errors += 1
            elif not link_name.is_symlink():
                # Allow real directories for certain tools (matching legacy behavior)
                if tool_name in ["qwen", "opencode"]:
                    continue
                log_error(f"Path exists but is not a symlink in {tool_name}: {link_name}")
                errors += 1
        return errors
        ...
        for tool_name, config in self.manifest["tools"].items():
            ...
            for doc_name in docs:
                ...
                if not doc_path.exists():
                    ...
                    content = doc_path.read_text()
                    if not content.startswith("@AGENTS.md"):
                        log_warn(f"{doc_name} does not start with @AGENTS.md - Prepending...")
                        doc_path.write_text("@AGENTS.md\n\n" + content)

    def _infer_category(self, skill_name: str) -> str:
        s = skill_name.lower()
        if any(x in s for x in ["security", "privacy", "audit"]): return "Security"
        if any(x in s for x in ["test", "quality", "check"]): return "Quality"
        if any(x in s for x in ["doc", "readme"]): return "Documentation"
        if "api" in s: return "API Development"
        if any(x in s for x in ["coordination", "parallel", "goap", "decomposition", "intent", "classifier"]): return "Coordination"
        if any(x in s for x in ["db", "database", "devops", "cicd", "pipeline"]): return "DevOps"
        if any(x in s for x in ["ui", "ux"]): return "UI/UX"
        if "skill" in s: return "Meta"
        if any(x in s for x in ["search", "web"]): return "Research"
        if any(x in s for x in ["migration", "refactor"]): return "Migration"
        if "accessibility" in s: return "Accessibility"
        if any(x in s for x in ["shell", "script"]): return "Code Quality"
        return "General"
    ...
    def _update_agents_md(self):
        ...
        # Check for existing section or the older "## Skills"
        for i, line in enumerate(lines):
            if line.startswith("### Available Skills") or line.startswith("## Skills"):
                start_line = i
        ...
        if start_line == -1:
            ...
            end_line = len(lines)
            log_info("Section '### Available Skills' not found in AGENTS.md, appending to end.")
